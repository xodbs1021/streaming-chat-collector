...
4. 보류(CRITICAL/HIGH — reviewer든 security-reviewer든)면 developer로 되돌려 수정 → 재검수. **통과까지 루프.**
5. §3이 런타임/브라우저 관찰을 요구하면 실제 동작 확인까지.
...
3. **채택 시**: 템플릿 repo(정본)에 에이전트를 추가·커밋하고 각 프로젝트에 sync. CLAUDE.md 변경 이력에 기록.
