...
## 결정 기록 (ADR — 포트폴리오·블로그 소재)

대안을 기각하고 방향을 확정한 **중요 설계 결정**이 작업 중에 있었으면, `docs/decisions/YYYY-MM-DD-<슬러그>.md` 한 장(결정 / 기각한 대안 / 왜)을 **같은 PR에 포함**한다 — 방법론은 `ecc:architecture-decision-records` 스킬 참조. 소소한 수정에는 만들지 않는다(YAGNI).

> progress.md(RAM, 휘발·아카이브)와 다르다: ADR은 **영구 큐레이션**이다. "왜 이렇게 결정했나"를 사람이 나중에 읽을 형태로 남기는 곳 — 블로그·면접에서 사고 과정을 보여줄 원료가 여기 쌓인다. 대화에서만 확정되고 diff에 안 드러나는 결정일수록 반드시 남긴다(claude-mem 압축본은 추론 결이 손실될 수 있음).
