...
| 2026-07-20 | 전역 에이전트 층 신설 — 템플릿 repo `global-agents/`(정본) → install/sync가 `~/.claude/agents/` 배포. 리서치 에이전트 2종 추가: `research-collector`([rc] 수집·메타데이터 강제·공백 명시) · `research-evaluator`([re] 독립 검증·인용 실존 확인·100점 스코어카드·채택/보류/기각). reporter는 기각(보고서=메인 세션 작성). ADR-0001을 템플릿 repo로 이관(+ADR-0002) — ADR 위치 원칙: 하네스 결정=템플릿, 도메인 결정=프로젝트 | docs/decisions/ (제거), 템플릿 repo | 리서치는 프로젝트 무관(전역 필요) + 전역 디렉토리는 버전 관리 불가(정본=repo) — 생태계 전수 조사 후 얇은 자체 정의 확정 — 템플릿 10da945 |
