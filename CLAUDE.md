...
- `PostToolUse(Edit|Write)` → 코드/설정 편집 시 `.claude/.needs-audit` 플래그 생성(`_workspace/`·`.claude/`·`*.md` 제외).
- `Stop` → 플래그가 있으면 **타입체크 실패 또는 completion-auditor 미실행 상태로 턴 종료를 차단**(rc=2). 감사자가 검증 통과 후 플래그를 지워야 종료 가능. `chat-harness` Phase 6이 이 훅으로 강제된다.
- **무한루프 가드:** Stop 차단은 한 턴에 최대 3회(`.claude/.stop-block-count`), 소진 시 경고 후 통과(세션 벽돌 방지).
- **탈출구:** `CHAT_HOOKS=off`(전체) 또는 `ECC_DISABLED_HOOKS`에 `pre:bash:master-guard`·`post:edit:needs-audit`·`stop:verify` 나열.
